...
function ModuleOrganizer() {
  ...
  const handleRemoveCustomName = (sectionId) => {
    setSections((prevSections) =>
      prevSections.map((section) =>
        section.id === sectionId ? { ...section, name: "", showCustomName: false } : section
      )
    );
  };
  ...
  const handleSave = async () => {
    ...
    try {
        const response = await axios.post("http://localhost:8000/api/modules/create/", {
          module_id: moduleId,
          assignment_id: assignmentId,
          sections: sections.map(({ id, overall_marks, custom_name, feedbacks, tag_name }) => ({
            section_id: id,
            overall_marks,
            custom_name,
            feedbacks,
        ...
        if (response.status === 201) {
            alert("Module and assignment details saved successfully!");
        } else {
            alert("Failed to save module and assignment details. Please try again.");
        }
    } catch (err) {
        if (err.response) {
            alert(`Error: ${err.response.data.detail || 'Server error'}`);
        } else if (err.request) {
            alert("Error connecting to the server. Please check your API and try again.");
        } else {
            ...
            <ul className="dropdown-menu">
              {dropdownItems.map((item, index) => (
                <li key={index}>
                  <button
                    className="dropdown-item"
                    onClick={() => handleTagSectionAdd(item, document.getElementById('tagInput').value)}
                  >
                    {item}
                  </button>
                </li>
